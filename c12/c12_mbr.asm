         ...
         ;以下进入保护模式... ...
         jmp dword 0x0010:flush             ;16位的描述符选择子：32位偏移 zhongshu-comment 段选择子的值是0x0010，即0000 0000 0001 0000，位3~位15是段描述符在gdt中的索引值（即gdt中的第n个描述符，n从0开始），0x0010的位3~位15数值等于0x10，即2，所以段描述符在gdt中的偏移量是2*8，即第16个字节处，对应29~30行处的那个描述符

         [bits 32]
  flush:
         mov eax,0x0018
         mov ds,eax     ;zhongshu-comment 解释参考55行代码我写的注释

         mov eax,0x0008                     ;加载数据段(0..4GB)选择子
         mov es,eax     ;zhongshu-comment 解释参考55行代码我写的注释
         mov fs,eax     ;zhongshu-comment 解释参考55行代码我写的注释
         mov gs,eax     ;zhongshu-comment 解释参考55行代码我写的注释

         mov eax,0x0020                     ;0000 0000 0010 0000
         mov ss,eax     ;zhongshu-comment 解释参考55行代码我写的注释
         xor esp,esp                        ;ESP <- 0
