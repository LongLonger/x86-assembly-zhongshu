...
;--zhongshu-comment 331~351行 参考 P262的第5~9段 -----------------------------------------------------------------------------
...
;-------------------------------------------------------------------------------
fill_descriptor_in_ldt:                     ;在LDT内安装一个新的描述符 zhongshu-comment 该过程参考P269 第4段开始
                                            ;输入：EDX:EAX=描述符
                                            ;          EBX=TCB基地址
                                            ;输出：CX=描述符的选择子
         push eax
...
load_relocate_program:                      ;加载并重定位用户程序
    ...
    ;zhongshu-comment 474~519行 参考 P268 14.4.3 加载用户程序
         mov ecx,mem_0_4_gb_seg_sel
         mov es,ecx
      
         mov esi,[ebp+11*4]                 ;从堆栈中取得TCB的基地址 zhongshu-comment P268 顶部那几段

         ;以下申请创建LDT所需要的内存 zhongshu-comment 481~484行 参考 P268 倒数第4、5段
         ...
         call sys_routine_seg_sel:allocate_memory
         mov [es:esi+0x06],ecx              ;登记程序加载基地址到TCB中 zhongshu-comment 参考 P268 倒数第2段
  ...
  .b1:
    ...
    ;zhongshu-comment 521~ 参考 P269 14.4.4 创建局部描述附表
         ...
         ;安装头部段描述符到LDT中 
         mov ebx,esi                        ;TCB的基地址
         call fill_descriptor_in_ldt    ;zhongshu-comment 将上文刚才创建的描述符安装到LDT中。该过程在421行
...
;---zhongshu-comment 735~772行 参考 P265中下~266 14.4.2以上----------------------------------------------------------------------------
...
;-------------------------------------------------------------------------------
start:  ;zhongshu-comment 内核程序的入口点
         mov ecx,core_data_seg_sel          ;使ds指向核心数据段 
         mov ds,ecx
    ;zhongshu-comment 779~809行，用于在屏幕上显示初始的信息，包括一个欢迎信息和一个处理器品牌信息
         mov ebx,message_1                    
         ...
         mov ebx,cpu_brnd1
         call sys_routine_seg_sel:put_string
    ;zhongshu-comment 812~826行，参考P261 14.3.2 调用门的安装和测试。
         ;以下开始安装为整个系统 服务的调用门。特权级之间的控制转移必须使用门
         mov edi,salt                       ;C-SALT表的起始位置 
         mov ecx,salt_items                 ;C-SALT表的条目数量 zhongshu-comment 循环的次数
  .b3:
         push ecx   
         mov eax,[edi+256]                  ;该条目入口点的32位偏移地址 zhongshu-comment 将该条目(例程)的32位段内偏移地址传送到EAX寄存器，每个条目的长度是262字节，而它的偏移地址则位于256字节处，占4个字节至259字节处
         mov bx,[edi+260]                   ;该条目入口点的段选择子 zhongshu-comment 用于获取条目(例程)所在代码段的选择子，它位于条目内260字节处
         mov cx,1_11_0_1100_000_00000B      ;特权级3的调用门(3以上的特权级才
                                            ;允许访问)，0个参数(因为用寄存器
                                            ;传递参数，而没有用栈) 
         call sys_routine_seg_sel:make_gate_descriptor  ;zhongshu-comment 该过程位于331行，用于组装64位的调用门描述符，调用门描述符的格式见P259 图14-9
         call sys_routine_seg_sel:set_up_gdt_descriptor ;zhongshu-comment c13_core.asm也有该例程，一模一样，参考那里的就好
         mov [edi+260],cx                   ;将返回的门描述符选择子回填 zhongshu-comment 将823行set_up_gdt_descriptor过程返回的内容回填到salt表的条目内，以覆盖原先的代码段选择子，salt表见331行代码
         ...
         ;对门进行测试 
         mov ebx,message_2
         call far [salt_1+256]              ;通过门显示信息(偏移量将被忽略) zhongshu-comment 调用salt表的第一个系统调用，用于显示829行ebx的字符串。调用门的时候只用到了260~261字节的段选择子，256~259字节的段内偏移量将会被忽略。参考816、817行代码和P259的第四段
         ...
         mov ecx,0x46
         call sys_routine_seg_sel:allocate_memory
         call append_to_tcb_link            ;将任务控制块追加到TCB链表 zhongshu-comment 在735行，属于内核代码段的近过程
    ;zhongshu-comment 840~843行 参考 P266 14.4.2 使用栈传递过程参数
